...
| STD-PUB-002A | FC-001 | Landing course items show total course price. | P0 | Covered | [04-01-public-discovery.spec.ts](../../tests/e2e/04-01-public-discovery.spec.ts) — `landing course cards show total course price` |
...
| STD-AUTH-005 | FC-003 | My Roles entry point is reachable from the account menu. | P1 | Covered | [04-02-account-menu-access.spec.ts](../../e2e-tests/tests/04-02-account-menu-access.spec.ts) — `registered users can open My Roles from user menu` |
...
| STD-REG-015 | FC-004 / FC-005 | User can review submitted registration requests and statuses in a dedicated history view. | P0 | Covered (E2E) | [tests/e2e/04-19-rejection-history-visibility.spec.ts](../../tests/e2e/04-19-rejection-history-visibility.spec.ts) — `[STD-REG-015] submitted school-manager request appears in registration history panel` (history panel visible with role, status, and school name after navigate-away-and-return). |
...
| STD-ADM-005 | FC-007 | Rejected request persists rejection state and reason after refresh/filter changes. | P1 | Covered (API+E2E) | API: [03-rejections-and-listing.spec.ts](../../tests/api/03-rejections-and-listing.spec.ts). E2E: [tests/e2e/04-19-rejection-history-visibility.spec.ts](../../tests/e2e/04-19-rejection-history-visibility.spec.ts) — `[STD-ADM-005][STD-INT-002]` admin rejection → requester sees REJECTED status and rejection reason on /registration after re-navigation. |
...
| STD-MGR-008 | FC-008 | Manager can reject instructor request and see updated state/history. | P0 | Covered (API+E2E) | API: [03-rejections-and-listing.spec.ts](../../tests/api/03-rejections-and-listing.spec.ts). E2E: [tests/e2e/04-19-rejection-history-visibility.spec.ts](../../tests/e2e/04-19-rejection-history-visibility.spec.ts) — `[STD-MGR-008]` manager rejects instructor request → requester sees REJECTED status and rejection reason on /registration after re-navigation. |
| STD-MGR-009 | FC-008 | Manager can reject student request and see updated state/history. | P0 | Partial | API: [03-rejections-and-listing.spec.ts](../../tests/api/03-rejections-and-listing.spec.ts). E2E instructor-request rejection covered in [04-19-rejection-history-visibility.spec.ts](../../tests/e2e/04-19-rejection-history-visibility.spec.ts); student-route rejection E2E still unproven (student pairs use course-interest flow, not direct reject button in student tab). |
...
| STD-SCH-004 | FC-009 | Unauthorized user cannot update another school's profile. | P0 | Covered (API) | [tests/api/07-course-close-reopen.spec.ts](../../tests/api/07-course-close-reopen.spec.ts) — `[STD-SCH-004] manager cannot update a school profile outside authorized scope`. |
...
| STD-SYL-003 | FC-010 | Manager can create draft syllabus from scratch. | P0 | Covered (API) | [tests/api/06-syllabus-draft-revision-regression.spec.ts](../../tests/api/06-syllabus-draft-revision-regression.spec.ts) — `[STD-SYL-003] manager can create a draft syllabus from scratch and retrieve it`. |
...
| STD-SYL-007 | FC-010 | UI clearly distinguishes draft, final, and obsolete versions. | P1 | Covered | [04-13-syllabus-editor-save-draft.spec.ts](../../tests/e2e/04-13-syllabus-editor-save-draft.spec.ts) — `[STD-SYL-007]` publishes multiple revisions and asserts DRAFT/FINAL details plus obsolete entries in the catalog. |
...
| STD-CRS-005A | FC-011 | Manager course lists show total course price per student. | P1 | Covered (API) | [07-course-close-reopen.spec.ts](../../tests/api/07-course-close-reopen.spec.ts) — `[STD-CRS-005A] manager course lists expose total course price per student` |
| STD-CRS-005 | FC-011 | Manager can view course metadata and enrollment details after creation. | P1 | Covered (API) | [tests/api/07-course-close-reopen.spec.ts](../../tests/api/07-course-close-reopen.spec.ts) — `[STD-CRS-005] manager can view course metadata and enrollment details after creation`. |
...
| STD-ASN-002 | FC-013 | Manager can assign instructor to selected course. | P0 | Covered (API) | [07-course-close-reopen.spec.ts](../../tests/api/07-course-close-reopen.spec.ts) validates assignment creation, lead/wage persistence, and assignment-time overlap guard behavior. |
...
| STD-ASN-004 | FC-013 | Schedule conflicts are surfaced as understandable errors. | P0 | Covered (API) | [tests/api/07-course-close-reopen.spec.ts](../../tests/api/07-course-close-reopen.spec.ts) — `[STD-ASN-004] assignment-time schedule conflicts are surfaced with a clear error`. |
| STD-ASN-005 | FC-013 | Qualification or business-rule conflicts are surfaced as understandable errors. | P1 | Covered (API) | [tests/api/07-course-close-reopen.spec.ts](../../tests/api/07-course-close-reopen.spec.ts) — `[STD-ASN-005] assigning instructors to started courses requires agreed wage per hour` validates INV-18 guard messaging. |
...
| STD-NAV-005 | FC-014 | Instructor sees only instructor-appropriate navigation links (Dashboard and Courses); Dashboard shows "Under construction" placeholder; Courses navigates to `/instructor/courses` with the `instructor-courses-page` testId visible. | P0 | Covered | [04-11-role-based-navigation.spec.ts](../../e2e-tests/tests/04-11-role-based-navigation.spec.ts) — `[STD-NAV-005]` validates instructor sidebar scope, dashboard placeholder, and courses page testId. |
...
| STD-NAV-013 | FC-014 / FC-016 | On narrow public landing screens in LTR, the mobile menu sheet stays on the left edge, has no duplicate close button, and remains open while the language picker is in use. | P1 | Covered | [04-11-role-based-navigation.spec.ts](../../e2e-tests/tests/04-11-role-based-navigation.spec.ts) — `[STD-NAV-013]` validates LTR left-edge panel placement, absence of extra sheet close button, and sheet persistence during language selection. |
...
| STD-I18N-010 | FC-016 | On narrow dashboard screens in RTL, the mobile hamburger button and sidebar panel are anchored to the RTL start (right) edge. | P1 | Covered | [04-12-i18n-rtl.spec.ts](../../e2e-tests/tests/04-12-i18n-rtl.spec.ts) — `[STD-I18N-010]` validates mobile RTL sidebar and hamburger button positioning on the dashboard. |
...
| STD-I18N-007 | FC-016 | Localized rendering does not remove access to required actions on critical manager pages. | P1 | Covered (E2E) | [tests/e2e/04-12-i18n-rtl.spec.ts](../../tests/e2e/04-12-i18n-rtl.spec.ts) — `[STD-I18N-007]` validates that filter controls and page layout remain accessible on the manager member-requests page after switching to Hebrew RTL. |
...
| STD-INT-002 | FC-007 / FC-008 | Rejection reason and decision history remain visible to appropriate reviewer. | P1 | Covered (E2E) | [tests/e2e/04-19-rejection-history-visibility.spec.ts](../../tests/e2e/04-19-rejection-history-visibility.spec.ts) — `[STD-ADM-005][STD-INT-002]` and `[STD-MGR-008]` tests verify rejection reason and REJECTED status are visible to the requesting user on /registration after admin or manager rejection. |
| STD-INT-003 | FC-011 / FC-012 / FC-013 | DB constraint failures are translated into understandable user-facing errors. | P0 | Covered (API) | [tests/api/07-course-close-reopen.spec.ts](../../tests/api/07-course-close-reopen.spec.ts) — `[STD-INT-003]` tags on duplicate enrollment, duplicate assignment, and assignment schedule-conflict messaging; [tests/api/05-course-hourly-rate.spec.ts](../../tests/api/05-course-hourly-rate.spec.ts) covers invalid syllabus creation rejection. |
...
| STD-CIN-004 | FC-017 | Student dashboard lists the student's CourseInterest records with status. | P1 | Covered | [tests/e2e/04-16-course-interest.spec.ts](../../tests/e2e/04-16-course-interest.spec.ts) — `[STD-CIN-004]` verifies the dashboard My Interests list renders course entries with status. |
...
| STD-EXEC-035 | FC-022 | When all ACTIVE students are assessed, lesson transitions to LESSON_CONCLUDED and a PENDING payout obligation is created for each eligible instructor. | P0 | Covered (API) | [tests/api/14-lesson-conclusion.spec.ts](../../tests/api/14-lesson-conclusion.spec.ts) — `creates pending payout for lead instructor when lesson concludes`; [tests/api/17-manual-finance-workflows.spec.ts](../../tests/api/17-manual-finance-workflows.spec.ts) validates payout-queue visibility. |
...
| STD-EXEC-042 | FC-012 | Late enrollment charges the student's account using effective enrollment price (agreed discount price when present, otherwise list price). | P0 | Covered (API) | [tests/api/15-late-enrollment.spec.ts](../../tests/api/15-late-enrollment.spec.ts) — `charges student account and credits school account (§8)`; [tests/api/17-manual-finance-workflows.spec.ts](../../tests/api/17-manual-finance-workflows.spec.ts) — `supports discounted enrollment pricing and charges agreed amount at course start`. |
...
### Manual finance workflows (FC-012, FC-022)

| STD ID | PRD Ref | Required test | Priority | Status | Existing API link or gap note |
|---|---|---|---|---|---|
| STD-EXEC-057 | FC-012 | Manager can record a manual student top-up with payment metadata (method/reference) and append-only deposit transaction. | P0 | Covered (API) | [tests/api/17-manual-finance-workflows.spec.ts](../../tests/api/17-manual-finance-workflows.spec.ts) — `records manager top-up with metadata and creates a deposit transaction`. |
| STD-EXEC-058 | FC-012 | Manager can enroll with discounted agreed price (<= list price) and start-course charge uses agreed price. | P0 | Covered (API) | [tests/api/17-manual-finance-workflows.spec.ts](../../tests/api/17-manual-finance-workflows.spec.ts) — `supports discounted enrollment pricing and charges agreed amount at course start`. |
| STD-EXEC-059 | FC-022 | Manager can mark a PENDING instructor payout as PAID, capturing settlement metadata and linked transactions. | P0 | Covered (API) | [tests/api/17-manual-finance-workflows.spec.ts](../../tests/api/17-manual-finance-workflows.spec.ts) — `creates pending payout ... and lets manager mark it as paid`. |
| STD-EXEC-060 | FC-012/FC-022 | Manager dashboard financial summary returns school effective balance, pending payout aggregate, recent top-ups, and current student funding gaps in school scope. | P1 | Covered (API) | [tests/api/18-dashboard-financial-summaries.spec.ts](../../tests/api/18-dashboard-financial-summaries.spec.ts) — `returns manager financial summary including recent top-ups`. |
| STD-EXEC-061 | FC-022 | Instructor dashboard financial summary returns pending/paid payout totals and recent payout records for the selected school scope. | P1 | Covered (API) | [tests/api/18-dashboard-financial-summaries.spec.ts](../../tests/api/18-dashboard-financial-summaries.spec.ts) — `returns instructor payout summary with pending amount after lesson conclusion`. |
| STD-EXEC-062 | FC-012 | Student dashboard financial summary returns per-school effective balances and recent transaction history for the student's accounts. | P1 | Covered (API) | [tests/api/18-dashboard-financial-summaries.spec.ts](../../tests/api/18-dashboard-financial-summaries.spec.ts) — `returns student balances and transactions summary`. |
...
| STD-CRS-034 | FC-022 | Lead instructor can submit a FAIL assessment with attended=false for an enrolled student. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-034]` |
| STD-CRS-035 | FC-023 | Lead instructor can mark a DECLINED co-instructor as ABSENT during LESSON_UNDERWAY. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-035]` |
...
| STD-CRS-019 | FC-018 | Manager is blocked from starting a course that has no lead instructor (INV-18). | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-019]` |
| STD-CRS-020 | FC-018 | Manager is blocked from starting a course where an instructor has no agreed wage (INV-18). | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-020]` |
| STD-CRS-021 | FC-018 | Manager can start an OPEN course that passes all guards → course moves to STARTED. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-021]` |
| STD-CRS-022 | FC-018 | Manager can close an OPEN course from the course detail page. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-022]` |
| STD-CRS-023 | FC-018 | Manager can close a STARTED course from the course detail page. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-023]` |
| STD-CRS-024 | FC-018 | Manager can reopen a CLOSED course from the course detail page. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-024]` |

### FC-019 Lesson scheduling — E2E

| STD ID | PRD Ref | Required test | Priority | Status | Existing E2E link or gap note |
|---|---|---|---|---|---|
| STD-CRS-025 | FC-019 | Lead instructor can schedule a CANCELLED lesson by providing a date and location. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-025]` |
| STD-CRS-026 | FC-019 | Lead instructor can reschedule a SCHEDULED lesson → status resets to SCHEDULED with new date. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-026]` |
| STD-CRS-027 | FC-019 | Lead instructor sees a reschedule action available on a BELOW_CAPACITY lesson. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-027]` |
| STD-CRS-028 | FC-019 | Lead instructor can reschedule a BELOW_CAPACITY lesson → lesson moves back to SCHEDULED. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-028]` |
| STD-CRS-029 | FC-019 / FC-021 | Rescheduling a BELOW_CAPACITY lesson that has a PENDING suggestion supersedes that suggestion. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-029]` |
| STD-CRS-030 | FC-019 | Lead instructor can reschedule a CONFIRMED lesson before its date → lesson returns to SCHEDULED. | P1 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-030]` |

### FC-020 Non-lead presence hints — E2E

| STD ID | PRD Ref | Required test | Priority | Status | Existing E2E link or gap note |
|---|---|---|---|---|---|
| STD-CRS-031 | FC-020 | Non-lead instructor sees presence hint controls on a SCHEDULED future lesson. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-031]` |
| STD-CRS-032 | FC-020 | Non-lead instructor can confirm availability before the lesson date → presence shows CONFIRMED. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-032]` |
| STD-CRS-033 | FC-020 | Non-lead instructor can report unavailability before the lesson date → presence shows DECLINED. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-033]` |

### FC-012 Late enrollment — E2E

| STD ID | PRD Ref | Required test | Priority | Status | Existing E2E link or gap note |
|---|---|---|---|---|---|
| STD-CRS-036 | FC-012 | Manager can enroll a new student in a STARTED course before the first lesson reaches LESSON_UNDERWAY. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-036]` |

### FC-021 Below-capacity resolution — E2E

| STD ID | PRD Ref | Required test | Priority | Status | Existing E2E link or gap note |
|---|---|---|---|---|---|
| STD-CRS-037 | FC-021 | Lead instructor can submit a PROCEED_WITH_PARTIAL suggestion on a BELOW_CAPACITY lesson via UI. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-037]` |
| STD-CRS-038 | FC-021 | Manager can approve a PROCEED_WITH_PARTIAL suggestion → lesson advances to CONFIRMED. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-038]` |
| STD-CRS-039 | FC-021 | Lead instructor can submit a CLOSE_COURSE suggestion on a BELOW_CAPACITY lesson via UI. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-039]` |
| STD-CRS-040 | FC-021 | Manager can approve a CLOSE_COURSE suggestion → lesson CANCELLED and course CLOSED. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-040]` |

### FC-025 Course completion — E2E

| STD ID | PRD Ref | Required test | Priority | Status | Existing E2E link or gap note |
|---|---|---|---|---|---|
| STD-CRS-041 | FC-025 | Course auto-transitions to COMPLETED when all enrolled students receive PASS on the final lesson. | P0 | Covered | [tests/e2e/04-18-course-execution-lifecycle.spec.ts](../../tests/e2e/04-18-course-execution-lifecycle.spec.ts) — `[STD-CRS-041]` |
...
| FC-004 Submit school-manager registration request | Covered (E2E); role-selection-path submission and request-history visibility (post-submission status on /registration) are covered. |
| FC-005 Submit instructor or student requests | Partial; submission exists, but school-selection validation remains incomplete. |
...
| FC-007 Review and decide school-manager requests | **Covered (API+E2E)**; approval/rejection/authz invariants are API-covered; E2E covers panel flow and confirmed that rejection reason and REJECTED status persist visibly to requester after re-navigation. |
| FC-008 Review instructor and student requests separately | **Covered (API+E2E)**; instructor/student approval and rejection invariants are API-covered; E2E covers instructor rejection history visibility; student-tab rejection route (student pairs use course-interest cancel, no direct reject button) remains partially covered. |
| FC-009 Manage school profile data | Partial; profile edit persistence, cross-school profile-edit denial, and multi-school context switching are covered. Remaining gap: active coverage for full profile-display fields (current evidence is inactive). |
| FC-010 Manage syllabus lifecycle | Partial; from-scratch/template draft creation, draft revision, publish-to-final, draft deletion, and obsolete-state distinction are covered. Remaining gap: broader manager catalog breadth. |
| FC-011 Open a course from a final syllabus version | Partial; API coverage includes creation from FINAL syllabus versions, persisted start-date/capacity fields, hourly-rate rules, non-FINAL rejection messaging, course metadata/enrollment-details reads after creation, `OPEN → CLOSED → OPEN` close/reopen lifecycle, `STARTED → CLOSED` direct close, course-start hard/soft guards, and `STARTED → COMPLETED` auto-transition. Remaining gap: course-details E2E UX depth. |
...
| FC-013 Assign instructors to courses | Partial; manager-scope lookup, assignment creation, duplicate blocking, cross-school denial, closed-course rejection, persisted lead instructor/agreed-wage assignment data, assignment-time schedule-overlap errors, and started-course wage guard conflicts are API-covered. Remaining gap: broader E2E user-flow/error-message coverage. |
...
| FC-016 Support localized and RTL manager experiences | Covered (E2E); login, landing discovery, desktop and mobile RTL positioning, and critical manager member-requests page usability in Hebrew RTL are covered. Remaining gap: detailed action-level assertions on other manager pages in RTL. |
| FC-017 Express course interest (course-first student flow) | **Covered (API+E2E)** for logged-in interest creation, anonymous redirect/continuation, student cancel/reopen toggle behavior, manager pending-interest cancel flow, enrollment label feedback, and student dashboard My Interests visibility. |
| FC-018 Start a course | **Covered (API+E2E)**; hard-guard rejections (no lead, no wage, insufficient balance, duplicate STARTED), soft minimum-capacity override, STARTED lifecycle event, enrolled-student financial charge, and E2E manager start/close/reopen lifecycle coverage (STD-CRS-019 to STD-CRS-024) are covered. |
| FC-019 Schedule and reschedule a lesson | **Covered (API+E2E)**; scheduling, rescheduling, schedule-overlap guard, hint reset on reschedule, CONFIRMED-state guard, and E2E scheduling/rescheduling flows (STD-CRS-025 to STD-CRS-030) are covered. |
| FC-020 Provide advisory attendance hints | **Covered (API+E2E)**; student accept/decline, lead-restricted visibility, and non-lead presence hint flows (STD-CRS-031 to STD-CRS-033) are covered. |
| FC-021 Resolve below-capacity lessons | **Covered (API+E2E)**; PROCEED_WITH_PARTIAL and CLOSE_COURSE suggestions, duplicate-pending guard (INV-06), manager approval advancing to CONFIRMED or CLOSED, reschedule-supersedes behavior, and E2E suggestion/approval flows (STD-CRS-037 to STD-CRS-040) are covered. |
| FC-022 Submit student evaluations | **Covered (API+E2E)**; PASS and FAIL assessment E2E flows (STD-CRS-012, STD-CRS-013, STD-CRS-034) and API invariants are covered. |
| FC-023 Mark co-instructor absent | **Covered (API+E2E)**; ABSENT assignment and DECLINED-only guard are API-covered and E2E absent-marking flow (STD-CRS-035) is covered. |
| FC-024 Submit and manage refund requests | **Covered (API+E2E)**; student submission (STD-CRS-014, STD-CRS-015), manager approval panel (STD-CRS-016), and manager approval action (STD-CRS-017) are covered with API invariants. |
| FC-025 Complete a course | **Covered (API+E2E)**; automatic COMPLETED transition when all students resolved and course lifecycle event creation are API-covered, and E2E completion flow (STD-CRS-041) is covered. |

## 6. Highest-Priority Gaps to Implement Next

1. Syllabus manager catalog breadth coverage (STD-SYL-001).
2. Instructor assignment conflict/error-message E2E coverage — schedule and business-rule paths (STD-ASN-004E2E).
3. STD-MGR-009: student-course pair cancellation E2E (distinct from `cancelCourseInterestForManager` flow already covered in 04-17).
4. Registration required-fields validation coverage depth (STD-REG-014).
5. STD-INT-001: decision-state visibility after page refresh — in-session coverage exists but explicit post-reload assertion is missing.
6. STD-INT-005: concurrent enrollment/assignment integrity (no concurrency E2E coverage).
