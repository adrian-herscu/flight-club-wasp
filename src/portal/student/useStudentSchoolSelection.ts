...
const STORAGE_KEY = "student:selected-school-id";
const EVENT_NAME = "student-selection-changed";

type StudentSchoolLike = {
  id: string;
};
...
export function useStudentSchoolSelection<T extends StudentSchoolLike>(schools: T[]) {
  const [selectedSchoolId, setSelectedSchoolId] = useState<string | null>(() => readStoredSchoolId());

  useEffect(() => {
    const handleSelectionChange = (event: Event) => {
      const customEvent = event as CustomEvent<{ schoolId: string | null }>;
